...
def get_attr_scores_all_runs():
    ...
    #FOR MULTIPLE RUNS
    fc_g_attr_files = np.unique(np.array(fc_g_attr_files))
    ...
    for model_attr in attr_dicts:
        ...
        tf_attr_dicts.append(tf_attr_dict)
    ...
    #attr_df = pd.DataFrame(tf_attr_mean_dict,index=input_genes)
    attr_dfs = [pd.DataFrame({tf:np.mean(d[tf],axis=0) for tf in d.keys()},index=input_genes) for d in tf_attr_dicts]
