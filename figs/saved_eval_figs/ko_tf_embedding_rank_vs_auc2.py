...
def all_sample_box_plot(ax):
    sns.boxplot(data=ae_auc_df[ae_auc_df['cutoff'] == 0],x='model',y='AUC',ax=ax,color='w',showfliers=False)
    sns.swarmplot(data=ae_auc_df[ae_auc_df['cutoff'] == 0],x='model',y='AUC',ax=ax,color='slategrey')
...
viper_activities = pd.read_csv('/nobackup/users/schaferd/ko_eval_data/data/regulons_QC/B1_perturbations/scenic_viper_TF_activities/VIPERdiff_activities.csv',sep='\t',index_col=0)
scenic_activities = pd.read_csv('/nobackup/users/schaferd/ko_eval_data/data/regulons_QC/B1_perturbations/scenic_viper_TF_activities/SCENICdiff_activities.csv',sep='\t',index_col=0)
...
d_id_to_kotf = pd.read_pickle('/nobackup/users/schaferd/ko_eval_data/data/regulons_QC/B1_perturbations/scenic_viper_TF_activities/id_to_kotf.pkl')
...
def dorothea_all_sample_box_plot(ax):
    sns.boxplot(data=dorothea_ae_df,x='model',y='AUC',ax=ax,color='w',showfliers=False)
    sns.swarmplot(data=dorothea_ae_df,x='model',y='AUC',ax=ax,color='slategrey')

    ax.axhline(y=viper_auc,color='green',linestyle='--',zorder=0,label='viper')
    ax.axhline(y=scenic_auc,color='blue',linestyle='dashdot',zorder=0,label='AUCell')
    ax.axhline(y=viper_s_s_auc,color='black',linestyle='dotted',zorder=0,label='viper-S-S Consensus')
