...
fc_g_train = fc_g_train[:5000]
s_s_train = s_s_train[:5000]

#np.savetxt('/home/schaferd/ae_project/Modular_DSCA_TF_Prediction/eval_saved_models/outputs/shallow_shallow/agg_data.pkl.activities.csv',s_s_train,delimiter='\t')
#np.savetxt('/home/schaferd/ae_project/Modular_DSCA_TF_Prediction/eval_saved_models/outputs/shallow_shallow/combined_processed_df.pkl.activities.csv',s_s_encode,delimiter='\t')
#np.savetxt('/home/schaferd/ae_project/Modular_DSCA_TF_Prediction/eval_saved_models/outputs/fc_g/agg_data.pkl.activities.csv',fc_g_train,delimiter='\t')
#np.savetxt('/home/schaferd/ae_project/Modular_DSCA_TF_Prediction/eval_saved_models/outputs/fc_g/combined_processed_df.pkl.activities.csv',fc_g_encode,delimiter='\t')
...
pca_embedding = PCA(n_components=30).fit_transform(results)

pca_tsne_embedding = TSNE(n_components=2).fit_transform(pca_embedding)

print(pca_embedding)
plt.scatter(pca_embedding[no_encode:,0],pca_embedding[no_encode:,1],marker='.',alpha=0.1)
plt.scatter(pca_embedding[:no_encode,0],pca_embedding[:no_encode,1],marker='.')
plt.savefig('pca.png')
plt.clf()

plt.scatter(pca_tsne_embedding[no_encode:,0],pca_tsne_embedding[no_encode:,1],marker='.',alpha=0.1)
