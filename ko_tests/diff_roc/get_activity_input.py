...
class ActivityInput():
    ...
    def get_activities(self):
        ...
        diff_df = pd.DataFrame(data=diff_embedding,index=pert_tfs[:4],columns=self.tf_list)
    ...
    def get_pos_neg_matrices(self):
        ...
        for i,pos in enumerate(self.positive_samples):
            ...
            if i == 3:
                break
        pos_matrix = torch.from_numpy(np.array(pos_df).astype(np.float)).to(device)
        neg_matrix = torch.from_numpy(np.array(neg_df).astype(np.float)).to(device)
